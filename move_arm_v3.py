...
class move_arm:
    ...
    ########################################
    #Initial pen writing : For 45 degree 
    #ARM移動関数（座標指定)
    def move_xyz_45(self):
        #座標->tilt変換してErrorでなければ移動させる
        self.xyz2tilt_45()
        if self.Error_Coordinate==0:
            self.move_tilt()
    ...
    def xyz2tilt(self):
        ...
        L_23 = 0.135	# Shoulder(id2)～Elbow(id3)のLink長さ
        L_34 = 0.176	# Elbow(id3)〜Wrist(id4)のLink長さ
        L_4G = 0.055	# Wrist(id4)〜Gripper_link先のLink長さ
        M_PI = math.pi  #3.14
    ...
    ########################################
    #Initial pen writing : For 45degree down
    #xyz座標->Tilt(SERVO角度)変換関数
    def xyz2tilt_45(self):
        #crane2+ ARM距離パラメータ
        L_B2 = 0.0710   # base_link〜Shoulder(id2)のLink長さ
        L_23 = 0.135    # Shoulder(id2)～Elbow(id3)のLink長さ
        L_34 = 0.176    # Elbow(id3)〜Wrist(id4)のLink長さ
